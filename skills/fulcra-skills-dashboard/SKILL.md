...
3. **Data Ingestion (Requires Consent):** Fetch the user's relevant Fulcra data using the `fulcra-api` CLI. 
   - **Important:** Always explicitly ask the user for permission to query the Fulcra API and fetch their records to build the dashboard *before* running any data commands.
   - **Discover Annotations:** Run `uv tool run fulcra-api catalog` to check for any user-created annotations. Annotation data types will be identified by the pattern `*Annotation/${ANNOTATION_ID}` (e.g., `Annotation/00000000-0000-0000-0000-000000000000`).
