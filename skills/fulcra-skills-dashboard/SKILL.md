...
2. **Theming (Agent Task):** Because the scaffolded dashboard is deliberately un-styled, you must heavily stylize it based on the user's preference. 
   ...
   - **Animations & Title Effects:** Inject pure CSS animations into the components to make the dashboard feel alive. Specifically, you **must** add a themed CSS animation to the main dashboard title (`<h1>`) in `HeroHeader.svelte` (e.g., a breathing swamp glow, a neon flicker, or a subtle floating effect). Keep all animations strictly within the `<style>` blocks using standard `@keyframes` to safely avoid breaking SvelteKit's compiler—do not use JS-based animation libraries or inline style strings.
