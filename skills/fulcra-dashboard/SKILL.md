...
2. **Data Ingestion (Requires Consent):** Automatically fetch the user's relevant Fulcra data using the `fulcra-api` CLI. 
   ...
   - **Agent Memory Dashboard Status:** Automatically check if the user has agent backup files stored in Fulcra. Run `uv tool run fulcra-api file list "agent/<lowercase-agent-name>/memory/"` to check for `memory.gz` or `top_of_mind.md`. If they exist, the dashboard template natively supports an "Agent Vault" module. Simply ensure you populate the UI with the file names and modified dates. **Crucial:** You must add the `local-only` class to the Agent Vault module so it hides on public deployments. Additionally, wrap the file names in an `<a>` tag pointing to `/api/download?file=<full-file-path>` so that clicking the file downloads it via the Python backend.
   - The `data.json` config file acts as a manifest. It should map your layout to the `.jsonl` files you downloaded, and you **must** include the annotation `description` in the timeline block, like this: `{"summary": "A concise overview of the current data and recent activity...", "timelines": [{"id": "...", "title": "...", "description": "The description from the catalog...", "icon": "...", "color": "...", "data": "timeline_name.jsonl"}], "recordsProcessed": "records_processed.jsonl"}`. Do not write meta-descriptions like "This is a retro dashboard," just summarize what is happening with the data. 
...
3. **Theming & Visualization:**
   ...
   - **Embrace the Theme (HTML & Copy):** Do not leave default boilerplate intact! Modify `index.html` directly to rewrite the main title, subtitle, and all component headers to fit the theme (e.g., change "Fulcra Dashboard" to "The Cybernetic Core", "Records Processed" to "Baguettes Baked", and "Sub-Surface Relay" to something thematically appropriate for the chat interface like "The Oracle's Ear" or "Comms Link"). Replace all default emojis (like 📊 or 🛰️) with theme-appropriate icons. You may alter the HTML structure to add new thematic containers or elements.
...
6. **Chat Envoy & GitHub Deployment:** 
   - The dashboard includes a chat envoy component (default title "Sub-Surface Relay"). Remind the user that this envoy only functions locally via the Python server.
